...
namespace Langben.App.Controllers
{
    ...
    /// 语言能力
    /// </summary>
    public class LanguageCompetenceApiController : BaseApiController
    {
        /// <summary>
        /// 异步加载数据
        /// </summary>
        ...
        {
            int total = 0;
            List<LanguageCompetence> queryData = m_BLL.GetByParam(null, getParam.page, getParam.rows, getParam.order, getParam.sort, getParam.search, ref total);
        ...
        /// <returns></returns>  
        public Common.ClientResult.Result Put([FromBody]LanguageCompetence entity)
        {
            Common.ClientResult.Result result = new Common.ClientResult.Result();
            if (entity != null && ModelState.IsValid)
            {   //数据校验
                ...
                else
                {
                    ...
                    LogClassModels.WriteServiceLog(Suggestion.UpdateFail + "，语言能力信息的Id为" + entity.Id + "," + returnValue, "语言能力"
                        );//写入日志   
                    result.Code = Common.ClientCode.Fail;
        ...
        public LanguageCompetenceApiController()
            : this(new LanguageCompetenceBLL()) { }

        public LanguageCompetenceApiController(LanguageCompetenceBLL bll)
        {
            m_BLL = bll;
        }
